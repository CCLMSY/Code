...
void solve()
{
    ...
    {
        cin >> t;
        if(t>0) posi.emplace_back(t);
        else if(t<0) nega.emplace_back(t);
        else cnt0++;
    }
    ll ans=cnt0;
    ll np=posi.size(),nn=nega.size();
    FORLL(i,0,np-1)
        if(posi[i]<=np-1-i){ //reachable
            ll l=0,r=nn-1;
            while(l<r){
                ll mid=(l+r)/2;
                if(mid>=posi[i]-nega[mid]) r=mid;
                else l=mid+1;
            }
            l = max(l-3,0ll);
            while(l<nn && l<posi[i]-nega[l]) l++;
            chmax(ans,cnt0+(i+1)+(nn-l));
    }
    FORLL(i,0,nn-1)
        if(-nega[i]<=i){ //reachable
            ll l=0,r=np-1;
            while(l<r){
                ll mid=(l+r)/2;
                if((np-1-mid)>=posi[mid]-nega[i]) r=mid;
                else l=mid+1;
            }
            r=min(r+3,np-1);
            while(r>=0 && (np-1-r)<posi[r]-nega[i]) r--;
            chmax(ans,cnt0+(nn-i)+(r+1));
    }
    cout << ans << endl;
}
/*----------Code Area----------*/
