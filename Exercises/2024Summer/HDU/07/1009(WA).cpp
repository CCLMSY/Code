...
struct obj{
    ...
    ll cost; //单个花费
    ll req; //需求数量
    vector<pll> son; //合成配方<id,cnt>
};
ll n,tg;
vector<obj> v;
void dfs(ll cur){
    if(v[cur].fl){
        for(auto &[id,cnt]:v[cur].son){
            v[id].req+=v[cur].req*cnt;
            dfs(id);
            v[cur].cost+=v[id].cost*cnt;
            if(v[cur].cost>1e9) v[cur].cost=1e10;
        }
    }
}
void solve()
{
    ...
    FORLL(i,1,n){
        v[i].req=0;
        if(v[i].fl) v[i].cost=0;
    }
    dfs(tg);
    if(v[tg].cost>1e9) cout << "Impossible\n";
    else cout << v[tg].cost << endl;
}
/*----------Code Area----------*/
