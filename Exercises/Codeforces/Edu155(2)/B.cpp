...
{
    ll n;cin >> n;
    ll suma=0,sumb=0,mina=INF,minb=INF;
    FORLL(i,1,n) {cin >> a[i];suma+=a[i];mina=min(mina,a[i]);}
    FORLL(i,1,n) {cin >> b[i];sumb+=b[i];minb=min(minb,b[i]);}
    cout << min(suma+minb*n,sumb+mina*n) << endl;
}
/*----------Code Area----------*/
