...
void solve()
{
    ll n,c,k,t;cin >> n >> c >> k;
    string s;cin >> s;
    vector<int> v;
    for(auto c:s) v.emplace_back(c-'A');
    vector<ll> rec(n);
    ll cur=0;
    unordered_map<int,ll> vcur;
    FORLL_rev(i,n-1,n-k){
        vcur[v[i]]++;
        if(vcur[v[i]]==1) cur|=pow2[v[i]];
        rec[i]=cur;
    }
    FORLL_rev(i,n-k-1,0){
        vcur[v[i+k]]--;
        if(vcur[v[i+k]]==0) cur^=pow2[v[i+k]];
        vcur[v[i]]++;
        if(vcur[v[i]]==1) cur|=pow2[v[i]];
        rec[i]=cur;
    }
    ...
    ll ans=c;
    FORLL(dic,1,ub){
        t = __builtin_popcount(dic);
        if(t>=ans) continue;
        if(!(rec[n-1]&dic)) continue;
        FORLL_rev(i,n-k,0){
            if(!(rec[i]&dic)) break;
            if(i==0){
                chmin(ans,t);
