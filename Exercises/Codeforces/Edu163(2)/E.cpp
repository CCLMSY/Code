...
void solve()
{
    ll n,k;cin >> n >> k;
    ll ans=(n-1)/k+1;
    vector<ll> asi(n+1,0),grp(n+1,0);
    FORLL(i,0,ans-1){
        FORLL(j,1,k) if(i*k+j<=n){
            grp[i*k+j]=i+1;
        }
        ll ub = min(k,n-i*k);
        FORLL(j,1,ub){
            asi[i*k+j]=(j+ub/2-1)%ub+1+i*k;
        }
    }
    FORLL(i,1,n) cout << asi[i] << Presentation(i,n);
    cout << ans << endl;
    FORLL(i,1,n) cout << grp[i] << Presentation(i,n);
}
/*----------Code Area----------*/
