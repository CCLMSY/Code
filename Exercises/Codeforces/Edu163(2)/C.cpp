...
void solve()
{
    ...
    int vstd[2][n][2]={0};
    for(int i=0;i<2;i++) for(int j=0;j<n;j++) vstd[i][j][0]=vstd[i][j][1]=0;
    set<pll> q[2];
    int fl=0;
    q[fl].insert({0,0}); vstd[0][0][0]=1;
    //0主动，1被动
    while (!q[fl].empty())
    {
        if(fl){
            while(!q[fl].empty()){
                ...
                if(!vstd[tp.first][tp.second][fl^1]){
                    vstd[tp.first][tp.second][fl^1]=1;
                    q[fl^1].insert(tp);
                }
            }
        }else{
            while(!q[fl].empty()){
                ...
                tp[2]={x,y+1};
                FORLL(i,0,2){
                    if(tp[i].second>=0&&tp[i].second<n)
                        if(!vstd[tp[i].first][tp[i].second][fl^1]){
                            vstd[tp[i].first][tp[i].second][fl^1]=1;
                            q[fl^1].insert(tp[i]);
                        }
                }
            }
