...
void solve()
{
    ll n,q,x,y,t1,t2,t;cin >> n;
    map<ll,ll> m;pll tp;auto mit=m.begin();
    FORLL(i,1,n){
        cin >> t;
        m[t]++;
    }
    cin >> q;vector<ll> re(q,0);ll cnt1,cnt2;
    ll delta,delta2;
    FORLL(i,1,q){
        cin >> x >> y;
        delta2=x*x-4*y;
        if(delta2<0) {re[i-1]=0;continue;}
        delta=sqrt(delta2);
        if(delta*delta!=delta2) {re[i-1]=0;continue;}
        if((x+delta)%2) {re[i-1]=0;continue;}
        t1=(x+delta)/2;t2=(x-delta)/2;cnt1=cnt2=0;
        cnt1=m[t1];
        cnt2=m[t2];
        if(delta==0){
            if(cnt1>=2) re[i-1]=cnt1*(cnt1-1)/2;
            else re[i-1]=0;
        }
        else re[i-1]=cnt1*cnt2;
    }
    FORLL(i,0,q-1){
        cout << re[i] << Presentation(i,q-1);
    }
}
/*----------Code Area----------*/
