...
void get_union(vector<ll> &U,vector<ll> S){
    SORT(U);SORT(S);
    ll r1=U.size(),r2=S.size();
    ll it1,it2;it1=it2=0;
    while(it1<r1&&it2<r2){
        if(U[it1]==S[it2]) {it1++;it2++;}
        else if(U[it1]<U[it2]) it1++;
        else {U.emplace_back(S[it2]);it2++;}
    }
    while(it2<r2) {U.emplace_back(S[it2]);it2++;}
}
...
void solve()
{
    ...
    for(auto x:s){
        FORLL(j,1,n){
            if(!mp[j][x]){
                FORLL(i,1,50) if(mp[j][i]) res.insert(i);
            }
