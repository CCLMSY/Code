...
ll f(ll i,ll y){
    if(v[i]>=y) return 0;
    if(i==n-1) return INF;
    return y-v[i]+f(i+1,y-1);
}
void solve()
{
    v.clear();
    cin >> n >> k;
    FORLL(i,1,n) {cin >> t;v.emplace_back(t);}
    ll l=t,r=(int)2e8;ll m=(l+r)/2;bool flag;
    while(r>l){
        FORLL(i,0,n-1) if(f(i,m)<=k) {l=m+1;m=(l+r)/2;break;}
        r=m-1;m=(l+r)/2;
    }ll re=0;
    FORLL(i,-10,10)
        FORLL(j,0,n-1) if(f(j,m+i)<=k) {re=m+i;continue;}
    cout << re << endl;
}
/*----------Code Area----------*/
