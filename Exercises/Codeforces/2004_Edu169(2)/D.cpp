...
void solve()
{
    ...
    while(q--){
        cin >> a >> b;
        if(a>b) swap(a,b);
        if(v[a]!=(15-v[b])){ cout << b-a << endl; continue;} //两点存在同色
        // if(r[a]==INF&&l[b]==0){ cout << "-1" << endl; continue;} //a右边无跳板，b左边无跳板
        // if(r[a]<=b||l[b]>=a) cout << b-a << endl; //a右边有跳板，b左边有跳板
        ll ans=INF;
        if(r[a]!=INF) chmin(ans,abs(r[a]-a)+abs(r[a]-b)); //a右边有跳板
        if(l[b]!=0) chmin(ans,abs(b-l[b])+abs(a-l[b])); //b左边有跳板
