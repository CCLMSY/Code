...
void solve()
{
    ...
    create_vec(v,n);
    string s;cin >> s;
    vector<ll> pref; pref.emplace_back(0);
    FORLL(i,0,n-1) pref.emplace_back(pref.back()+v[i]);
    deque<ll> posl,posr;
    FORLL(i,0,n-1)
        if(s[i]=='L') posl.emplace_back(i+1);
        else posr.emplace_back(i+1);
    ll ans=0;
    while(!posl.empty()&&!posr.empty()){
        if(posl.front()>posr.back()) break;
        ans+=pref[posr.back()]-pref[posl.front()-1];
        posl.pop_front(); posr.pop_back();
    }
    cout << ans << endl;
}
/*----------Code Area----------*/
