...
struct marble{
    ll a,b,val;
    bool operator < (const marble &x) const{
        return val<x.val;
    }
};
...
void solve()
{
    ll n;cin >> n;
    vector<marble> v(n);
    FORLL(i,0,n-1) cin >> v[i].a;
    FORLL(i,0,n-1) cin >> v[i].b;
    FORLL(i,0,n-1) v[i].val=v[i].a+v[i].b;
    SORT_REV(v);
    ll re=0;
    FORLL(i,0,n-1) if(i%2==0) re+=v[i].a-1;else re-=v[i].b-1;
    cout << re << endl;
}
/*----------Code Area----------*/
