...
void solve()
{
    ...
    multiset<ll>::iterator it=v.begin();
    ll pre=*it;it++;
    for(;it!=v.end();it++){
        em.emplace_back(*it-pre);
        pre=*it;
    }
    ...
    FORLL(iq,1,q){
        ...
        if(n==1) {re.emplace_back(tv[0]);continue;}

        ll t=0;
        SORT(tv);
        FORLL(i,0,n-2) t=max(t,tv[i+1]-tv[i]);

        re.emplace_back(tv[n-1]+t);
    }

    FORLL(i,0,q-1) cout << re[i] << Presentation(i,q-1);
}
/*----------Code Area----------*/
