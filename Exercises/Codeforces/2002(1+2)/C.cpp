...
namespace CCLIB
{
    ...
    ostream& operator<<(ostream &out, const pair<ll,ll> &p) {out << '(' << p.x << ',' << p.y << ')';return out;}
...
void solve()
{
    ...
    pll st,ed; cin >> st.x >> st.y >> ed.x >> ed.y;
    pll sted=pll(ed.x-st.x,ed.y-st.y);
    ll len = sted.x*sted.x+sted.y*sted.y;
    for(auto p:v){
        pll ped=pll(p.x-ed.x,p.y-ed.y);
        ll len2 = ped.x*ped.x+ped.y*ped.y;
        if(len2<=len){
            cout << "NO\n";
            return;
        }
