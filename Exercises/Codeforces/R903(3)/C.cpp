...
void solve()
{
    ...
    FORLL(i,1,n){
        FORLL(j,1,n) matrix[i][j]=getchar();
        getchar();
    }
    ll re=0;char tc;
    FORLL(i,1,n/2){
        FORLL(j,i,n-i){
            tc=max({matrix[i][j],matrix[n+1-j][i],matrix[j][n+1-i],matrix[n+1-i][n+1-j]});
            re+=tc*4-(matrix[i][j]+matrix[n+1-j][i]+matrix[j][n+1-i]+matrix[n+1-i][n+1-j]);
        }
    }cout << re << endl;
}
/*----------Code Area----------*/
