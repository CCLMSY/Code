...
void solve()
{
    ...
    ll i=1,pre1=0,pre2=0;
    bool f1=1,f2=1;
    while(f1&&f2){
        if(i&1){
            if(vn.front()<vm.back()){
                sumn+=vm.back()-vn.front();
                summ+=vn.front()-vm.back();
                swap(vn.front(),vm.back());
            }
            if(pre1==sumn) f1=0;
            else pre1=sumn;
        }else{
            if(vm.front()<vn.back()){
                sumn+=vm.front()-vn.back();
                summ+=vn.back()-vm.front();
                swap(vn.back(),vm.front());
            }
            if(pre2==sumn) f2=0;
            else pre2=sumn;
        }
