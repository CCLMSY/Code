...
void solve()
{
    ...
    FORLL(i,1,n) {
        cin >> t;
        v.emplace_back(t);
    }vector<ll> ov=v;SORT(v);vector<ll> dif=v;
    FORLL_rev(i,n-1,1) dif[i]-=dif[i-1]; dif[0]=0;dif.emplace_back(0);
    vector<ll> re(n);ll l=0,r=n-1,ans=n;
    FORLL(i,1,n){
        ans+=dif[i]*(r-i+1);
    }re[0]=ans;l++;r--;
    FORLL(i,1,n-1){
    ...
    FORLL(i,0,n-1){
        cout << re[lower_bound(ALL(v),ov[i])-v.begin()] << Presentation(i,n-1);
    }
}
/*----------Code Area----------*/
