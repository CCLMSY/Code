...
void solve()
{
    ...
    s.clear();
    while(y<n&&s.size()<x) if(v[y++]<x) s.insert(v[y-1]);
    if(y>=n) {cout << "-1" << endl;return;}
    s.clear();
    FORLL(i,y,n) if(v[i]<x) s.insert(v[i]);
    if(s.size()<x) {cout << "-1" << endl;return;}
    cout << 2 << endl;
    cout << 1 << ' ' << y << endl;
    cout << y+1 << ' ' << n << endl;
}
