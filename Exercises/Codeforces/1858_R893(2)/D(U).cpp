...
void solve()
{
    ll n,k;string s;
    cin >> n >> k;cin >> s;
    vector<ll> pos0,pos1,len0,len1;char c=s[0];ll t=1;
    if(c=='0') pos0.emplace_back(0);else pos1.emplace_back(0);
    FORLL(i,1,n-1){
        if(s[i]==c) t++;
        else{
            if(c=='0'){
                len0.emplace_back(t);t=1;
                pos1.emplace_back(i);c='1';
            }else{
                len1.emplace_back(t);t=1;
                pos0.emplace_back(i);c='0';
            }
        }
    }if(c=='0') len0.emplace_back(t); else len1.emplace_back(t);
    t=pos0.size();
}
/*----------Code Area----------*/
